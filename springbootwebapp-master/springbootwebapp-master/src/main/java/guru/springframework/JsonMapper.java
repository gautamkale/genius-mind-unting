...
public class JsonMapper {
	...
	public List<String> mapProfileJson(String filePath) throws JsonParseException, JsonMappingException, IOException{
        ObjectMapper mapper = new ObjectMapper();
		TypeReference<List<String>> typeReference = new TypeReference<List<String>>(){};
		File initialFile = new File(filePath);
		List<String> profileLink= new ArrayList<>();
	    InputStream inputStream = new FileInputStream(initialFile);
		///InputStream inputStream = TypeReference.class.getResourceAsStream("/json/testJson.json");
		try {
			profileLink = mapper.readValue(inputStream,typeReference);
		} catch (IOException e){
			System.out.println("Unable to save users: " + e.getMessage());
		}
		 return profileLink;
